...
signed main(){
    ...
    for (int i = 1; i <= n; i++){
        ...
        maxde = min(maxde, maxd(a[i]));
        ans = add(ans,a[i].fi - 1);
        maxup = min(maxup, a[i].fi - a[i].se);
        maxdown = min(maxdown, a[i].se - 1);
    ...
    while(!lists.empty()){
        ...
        ans2 = add(ans2, mul(subtr(min(maxde,val), lastVal), fastpow(2,(int)lists.size())));
    ...
    if (!ans2){
        ans = add(ans, maxup + maxdown);
    }
    ans = add(ans, ans2);
    ans = add(ans, max(0ll,maxde - lastVal));
    cout << ans;
